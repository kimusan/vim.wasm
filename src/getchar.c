...
#define DUM_LEN MAXMAPLEN * 3 + 3
static void (*inchar_async_callback_)(int);
static char_u inchar_async_got_int_dum_[DUM_LEN + 1];
static int inchar_async_got_int_retesc_;
static int inchar_async_tb_change_cnt_;
static char_u *inchar_async_buf_;

static void
inchar_async_got_int_callback(int len)
{
    if (len == 0 || (len == 1 && inchar_async_got_int_dum_[0] == 3)) {
	// Try again
	ui_inchar_async(inchar_async_got_int_dum_, DUM_LEN, 0L, 0, inchar_async_got_int_callback);
	return;
    }
    inchar_async_callback_(inchar_async_got_int_retesc_);
}

static void
inchar_async_inchar_async_fillup_callback(int len)
{
    /* If the typebuf was changed further down, it is like nothing was added by
     * this call. */
    if (typebuf_changed(inchar_async_tb_change_cnt_)) {
	inchar_async_callback_(0);
	return;
    }
    ...
    inchar_async_callback_(fix_input_buffer(inchar_async_buf_, len));
}
...
inchar_async(
    ...
    int		maxlen,
    long	wait_time,
    void	(*callback)(int))	    /* milli seconds */
{
    int		len = 0;	    /* init for GCC */
    int		retesc = FALSE;	    /* return ESC with gotint */
    int		script_char;
    ...
    inchar_async_callback_ =  callback;
	...
	if (got_int)
	{
	    inchar_async_got_int_retesc_ = retesc;
	    ui_inchar_async(inchar_async_got_int_dum_, DUM_LEN, 0L, 0, inchar_async_got_int_callback);
	    return;
	}
	 ...
	 * tripled below.
	 */
	inchar_async_tb_change_cnt_ = tb_change_cnt;
	inchar_async_buf_ = buf;

	ui_inchar_async(buf, maxlen / 3, wait_time, tb_change_cnt, inchar_async_inchar_async_fillup_callback);
	return;
    }
    ...
    callback(fix_input_buffer(buf, len));
}
#endif /* FEAT_GUI_WASM */
