...
static void
inchar_async_finish(int len)
{
    /* If the typebuf was changed further down, it is like nothing was added by
     * this call. */
    if (typebuf_changed(inchar_async_state.tb_change_cnt)) {
	inchar_async_state.callback(0);
	return; // return 0;
    }
...
inchar_async(char_u *buf, int maxlen, long wait_time, void (*callback)(int))
{
    ...
    inchar_async_state.tb_change_cnt = typebuf.tb_change_cnt;
...
#define DUM_LEN MAXMAPLEN * 3 + 3
	 ...
	 * tripled below.
	 */
	ui_inchar_async(inchar_async_state.buf, maxlen / 3, wait_time, inchar_async_state.tb_change_cnt, inchar_async_finish);
	return;
    }

    inchar_async_finish(len);
}
#endif
